...
func (c *WarehouseController) GetWarehouses() {
	var (
	...
	rets := models.GetWarehouseBypage(pageNum, pageSize)
	util.RetContent.Code = util.SUCESSFUL
	util.RetContent.Data = rets
	c.Data["json"] = util.RetContent
	c.ServeJSON()
...
func (c *WarehouseController) GetWarehouseById() {
	var (
		param = make(map[string]int64)
	)
	err := json.Unmarshal(c.Ctx.Input.RequestBody, &param)
	...
	ret, _ := models.GetWarehouseById(id)
	util.RetContent.Code = util.SUCESSFUL
...
func (c *WarehouseController) EditWarehouseById() {
	var (
		param models.Warehouse
	)
	...
	code := models.EditWarehouseById(param)
...
func (c *WarehouseController) AddWarehouse() {
	var (
		param models.Warehouse
	)
	...
	code := models.AddWarehouse(param)
	util.RetContent.Code = code
	c.Data["json"] = util.RetContent
	c.ServeJSON()
}
...
func (c *WarehouseController) DeleteWarehouse() {
	...
	code := models.DeleteWarehouse(id)
	util.RetContent.Code = code
	c.Data["json"] = util.RetContent
	c.ServeJSON()
}
